 ...
 *
 *  Changes (1):
 *    - Create table [dbo].[testCompanies]
 *
 ...
 *
 *  FILE       : auto_ERP_System__dbo.testCompanies__table_create.sql
 ...
 *  sqlcmd -S 192.168.10.200 -d ERP_System -U YourSqlLogin -P "YourPassword" -C -i ".\auto_ERP_System__dbo.testCompanies__table_create.sql"
 *
 *  # Windows auth (if TARGET trusts your Windows account):
 *  sqlcmd -S 192.168.10.200 -d ERP_System -E -C -i ".\auto_ERP_System__dbo.testCompanies__table_create.sql"
 *
...
BEGIN TRY
    ...
    -- Create table [dbo].[testCompanies]
    IF OBJECT_ID(N'[dbo].[testCompanies]', N'U') IS NULL
    BEGIN
        DECLARE @sql_create nvarchar(max) = N'';
    SET @sql_create = @sql_create + N'CREATE TABLE [dbo].[testCompanies](
    ...
    COMMIT TRANSACTION;
    PRINT 'SUCCESS: [ERP_System] [dbo].[testCompanies] table_create applied.';
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    PRINT 'FAILED: [ERP_System] [dbo].[testCompanies] table_create -> ' + ERROR_MESSAGE();
    THROW;
END CATCH
GO
