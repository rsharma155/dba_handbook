/*
================================================================================
Key Lookup Column Extraction from Plan Cache
================================================================================
Description:
    Parses cached plans for Index Lookup operators, extracts the lookup target index and lookup column list, and aggregates plans using each lookup pattern. Complements plan_cache_deep_dive.sql which only flags lookups.

Source Attribution:
    Adapted from Kendal Van Dyke's SQL-Server-Scripts
    https://github.com/kendalvandyke/SQL-Server-Scripts
    Original file: Performance - Find Key Lookups (2005+).sql
...
Action:
    Use Lookup_Columns to design covering INCLUDE columns on the nonclustered index. Prioritize by total_logical_reads / execution_count. Run off-peak — dm_exec_query_plan is expensive.
