# =============================================================================
# Module:   schema_compare/Compare-SqlSchema.ps1
# Purpose:  Core engine - compares user-object schemas between two SQL Server
#           databases and generates an ordered, reviewable T-SQL sync script.
# Author:   Ravi Sharma
# Created:  2026-05-22
...
# Avoid VT/ANSI sequences in captured GUI logs (PowerShell 7+).
...
function Export-HtmlReport {
    ...
    # Flatten carefully: empty Generic Lists must not become $null before counting,
    # and foreach-with-no-output is $null under PowerShell (StrictMode .Count throws).
    $allDiffs = @(foreach ($r in $AllResults) { foreach ($d in @($r.Differences)) { $d } })
    $total    = Get-ObjectCount $allDiffs
    ...
    $dbRows = foreach ($pair in $DatabasePairs) {
        ...
        $chgCount = if ($r -and $r.PSObject.Properties['Changes']) { Get-ObjectCount $r.Changes } else { 0 }
        "<tr><td>$(& $enc $srcDb)</td><td>$(& $enc $tgtDb)</td>" +
        "<td class='num'>$(Get-ObjectCount $dbDiffs)</td><td class='num'>$chgCount</td></tr>"
    }
...
foreach ($pair in $comparePairs) {
    ...
    $dbManifest = @()
    $changeCount = Get-ObjectCount $result.Changes
    $diffCount   = Get-ObjectCount $result.Differences
    if ($GenerateSyncScript -and $changeCount -gt 0) {
        ...
        $autoN   = Get-ObjectCount @($dbManifest | Where-Object { $_.Mode -eq 'Auto' })
        $manualN = Get-ObjectCount @($dbManifest | Where-Object { $_.Mode -eq 'Manual' })
        Write-Log "  Wrote $(Get-ObjectCount $dbManifest) script(s): $autoN auto_, $manualN manual_  -> $dbOutDir" 'Green'
    ...
    # Always emit a summary row (including zero-diff) so GUI / pipeline callers get stable shape.
    $summaries.Add([pscustomobject]@{
        Database        = $srcDbName
        TargetDatabase  = $tgtDbName
        DifferenceCount = $diffCount
        AutoScripts     = Get-ObjectCount @($dbManifest | Where-Object { $_.Mode -eq 'Auto' })
        ManualScripts   = Get-ObjectCount @($dbManifest | Where-Object { $_.Mode -eq 'Manual' })
...
$flatDiffs = @(foreach ($r in $allResults) { foreach ($d in @($r.Differences)) { $d } })

if ($ReportFormat -contains 'GridView' -and (Get-ObjectCount $flatDiffs) -gt 0) {
...
if ($ReportFormat -contains 'Console') {
    ...
    if ((Get-ObjectCount $flatDiffs) -eq 0) {
...
# Output as object array (not raw List) so one-to-many and single-summary
# callers both receive stable enumeration via @($summaries).
